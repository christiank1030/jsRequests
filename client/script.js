...
let helloColorRevert = (event => {
    sayHelloButton.style.backgroundColor = '#EFEFEF';
    sayHelloButton.style.color = 'black';
})

sayHelloButton.addEventListener('mouseout', helloColorRevert);
    ...
    axios.get('http://localhost:3000/animals')
        .then(response => {
            for(let i = 0; i < response.data.length; i++) {
                ...
                let currIndex = response.data[i];
                
                newP.textContent = currIndex;
...
/*
    ...
    Inside the repeatMyParam function above, grab the element with the id 
    of 'repeat-text' and set its textContent property equal to the response data.
...
/*
    ...
    Write a function that makes a get request to 'http://localhost:3000/query-test', with 
    a query of your choice on the end!
...
let attachQuery = (event => {
    axios.get(`http://localhost:3000/query-test?query=best-query&query=worst-query`) 
        .then(response => {
            console.log(response.data)
        })
})
...
/*
    ...
    The input field should have a placeholder that tells the user to enter a food. And the button should 
    indicate that it will add food into a list. 
    ...
    Next, create an object called body inside the function. It should have one key-value pair. The key should 
    be newFood (make sure to match the case and spelling exactly) and the value should be the value of the food input. 

    Now make an axios post request to /food. Inside the parentheses where you passed the URL in, pass in body 
...
let createFood = (event => {
    ...
    axios.post("http://localhost:3000/food", body)
        .then(response => {
            ...
            let userFood = document.createElement('p');

            userFood.textContent = response.data
            document.body.appendChild(userFood)
        })
})

document.querySelector("#addFood").addEventListener("submit", createFood)